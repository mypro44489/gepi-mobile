 ...
 */
class RetardController extends ActiveController {

    public $modelClass = 'api\modules\v1\models\Retard';
...
//     ORDER BY total_win DESC", [':start_date' => '1970-01-01']);
        $command = $connection->createCommand('SELECT retard.*, periode.HEUREPERIODE, user.nom, user.prenoms FROM retard JOIN user ON (retard.IDELEVE=user.ID) JOIN periode ON (retard.IDPERIODE=periode.ID)');
    ...
    public function actionAdd() {
        ...
        if ($postdata) {
            $model = new \api\modules\v1\models\Retard();
